...
from MyCommon import Util, AsmUtil, Enums, CryptoUtil
    ...
    def hexdump_calc(self):
        ...
        try:
            ...
            if "  " in inputdata:
                ...
                self.txthexdump_output.setPlainText(res)
            self.hexdump_cmb_change()
        except Exception as ex:
            self.appendLog("转换异常:" + str(ex))
            self.txthexdump_output.setPlainText("")

    # 输入的是从charles里面获取出来的那种hexdump格式
    def hexdump_calc2(self):
        inputdata = self.txthexdump_input.toPlainText()
        try:
            resBuff=Util.fromHexDump(inputdata)
            res = Util.ByteToHexStr(Util.b2hex(resBuff))
